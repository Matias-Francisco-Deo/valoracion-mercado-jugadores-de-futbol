...
All requirements MUST be backed by tests in the proper scope and order. Model tests MUST be unit tests without any database or Spring runtime. Service and repository tests MUST be integration tests using real PostgreSQL instances managed through Testcontainers. End-to-end API tests MUST use @SpringBootTest and RestClient and MUST live in their own package, never mixed with service tests. Test execution MUST proceed from the normal happy path to border cases, ensuring the most common behavior is proved before exceptional flows. Test names MUST clearly represent the behavior under test; if a test needs extra context, the author MUST add a clarifying comment.
